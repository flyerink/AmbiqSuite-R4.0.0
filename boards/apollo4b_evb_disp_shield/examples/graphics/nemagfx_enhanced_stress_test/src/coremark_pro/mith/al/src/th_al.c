...
#define DEBUG_PIN_0 (22)
#define DEBUG_PIN_1 (23)
 ...
 * Local Data
 */
static char gPrintfBuffer[PRINTFBUFFER_SIZE];
...
int		al_printf(const char *fmt, va_list	args)
{
    ...
    ui32NumChars = vsprintf(gPrintfBuffer, fmt, args);

    //
    // This is where we print the buffer to the configured interface.
    //
    g_pfnCharPrint(gPrintfBuffer);
...
char	*al_getenv( const char *key )
{
	...
	return getenv(key);
#else
	//TODO: Who will call this?May need to print the env variable here
   am_util_stdio_printf("getenv:%s", key);
	return NULL;
#endif
}

extern void *pvPortMalloc_external( size_t xWantedSize );
extern void vPortFree_external( void *pv );
extern size_t xPortGetBlockSize_external( void *pv );

void* al_malloc(size_t size)
{
   return pvPortMalloc_external(size);
}

void* al_malloc_aligned(size_t size, size_t align)
{
   //no need to worry about the align
   return pvPortMalloc_external(size);
}
...
void* al_realloc(void *ptr, size_t size)
{
   ...
   origin_size = xPortGetBlockSize_external(ptr);
   ...
   {
      new_ptr = pvPortMalloc_external(size);

      if(new_ptr != NULL)
      {
         memcpy(new_ptr, ptr, origin_size);
      }

      vPortFree_external(ptr);
   }
...
void	al_main( int argc, char* argv[]  )
{
   ...
   //std_in and std_out are not used, std_err was just remapped to our system console
	//redirect_std_files();
	...
	*/
#if 	!HOST_EXAMPLE_CODE
	//TODO: our console is ready, what we can do here

#ifdef USE_DEBUG_PIN
    am_hal_gpio_pinconfig(DEBUG_PIN_0, am_hal_gpio_pincfg_output);
    ...
    am_hal_gpio_output_clear(DEBUG_PIN_0);
