...
int      th_fprintf (ee_FILE *fp, const char* format, ...) 
{
   ...
   //we don't support std_in,std_out, std_err, we just printf them by system console.
   *fp = *fp;
   rv = al_printf(format, args );
   va_end( args );
   return rv;
}
/* Function: th_vfprintf
	...
	Note:
		Redirected to stdio in initial version of MITH
*/
int      th_vfprintf(ee_FILE *fp, const char *format, va_list ap){*fp = *fp; return al_printf(format, ap);}
/* Function: th_fseek
...
void *th_aligned_malloc_x( size_t size, size_t align, const char *file, int line )

   {
	   ...
	   file=file; 
	   line=line;
   p=al_malloc_aligned( size, align); /* file, line );*/
#if REPORT_THMALLOC_STATS
	if (size==0) {
		th_printf("Malloc of 0 from %s:%d\n",file,line);
	}
...
void th_aligned_free_x( void *block, const char *file, int line ) {
	...
	al_mutex_unlock(&th_malloc_mutex);
#endif
	al_free_aligned ( block ); /*, file, line );*/
}
...
ee_intlist *ee_list_add(ee_intlist **p, void *data, unsigned int info)
{
        ...
        ee_intlist *n = (ee_intlist *)al_malloc(sizeof(ee_intlist));
        if (n == NULL)
                return NULL;
