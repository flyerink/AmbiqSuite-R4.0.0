...
extern e_u32 mith_wl_get_uid(void* ptr_context);
...
int al_thread_create(al_thread_t * thread, void *(*start_routine)(void *), void * arg) {
	...
	//780641437 is the uid for the parser-125k workload, this workload needs a big stack size
	if(mith_wl_get_uid(arg) == 780641437)
		stacksize = PTHREAD_BIG_STACK_SIZE;
	else
		stacksize = PTHREAD_DEFAULT_STACK_SIZE;
