...
e_u32 mith_wl_get_uid(void* ptr_context)
{
	mith_context_t *context = (mith_context_t *)ptr_context;
	return context->workload->uid;
}
...
#if REPORT_THMALLOC_STATS 
	print_memstat("Max allocated memory",workload->shortname,(float)th_malloc_max);
        th_malloc_max = 0.0f;
#endif
